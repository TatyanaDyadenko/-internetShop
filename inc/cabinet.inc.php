<?php
/*
	Подключаемый функциональный модуль - cabinet.inc.php 
	Осуществляет формирование котента центральной части страницы "О нас"
	06.03.2020
*/
		...
		$cabinet = '
				...
				<!--form action=change.phtml method=get-->
					...
					<table border="0" width="100%" align="right">
						...
						<tr>
							<td>
								<i><input type="checkbox" value="1" name="subscribe"'.(($row["subscribe"]==1) ? "checked" : "").'>
							</td>
...
';
		...
		$strSQL1="SELECT id_order, date_ord
				FROM orders
				WHERE id_cust=".$id." 
		...
		while($row1=mysql_fetch_array($result1))
		{
			$order=$row1["id_order"];
			$strSQL2="
			SELECT author, name_book, pages, price, kolvo, id_order, books.id_book
...
';
			...
			$strSQL3="SELECT name_cat
					FROM categories, orders
					WHERE
						categories.id_cat=orders.bonus AND
						id_order='".$order."'";
			$result3=mysql_query($strSQL3) or die("He могу выполнить запрос [$strSQL3]!");
			if($row3=mysql_fetch_array($result3))
			{
				$cabinet .= '
					<tr>
						<td соlspan=2>Бесплатный каталог по теме </td>
						<td><b>'.$row3["name_cat"].'</b></td>
						<td></td>
						<td></td>
					</tr>
';
