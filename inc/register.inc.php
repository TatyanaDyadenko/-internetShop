<?php
/*
	Подключаемый функциональный модуль - register.inc.php 
	Осуществляет формирование котента центральной части страницы "Регистрация"
	06.03.2020
*/
	...
	if($fam!="" && $im!="" && $addr!="" && $mail!="" && $login!="" && $pass!="" && $pass2!="")
	{
		...
		else
		{
			...
			else
			{
				...
				//-- сессию стартуем в headers.inc.php: session_start(); уничтожим ее в exit.php
				$_SESSION['log'] = $fam." ".$im;
				...
				$message="<span bgcolor='#66cc66' align='center'> <b>Вы успешно 3apeгистрировались.</b></span>
							<p>Через 5 секунд Вы окажетесь в Личном Кабинете.</p>";
				$success=true;
			}
		}
	}
